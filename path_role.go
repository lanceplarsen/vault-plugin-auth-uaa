...
func pathRoleList(b *cfAuthBackend) *framework.Path {
...
func pathRole(b *cfAuthBackend) *framework.Path {
	return &framework.Path{
...
TTL will be set to the value of this parameter.`,
			...
			"bound_guids": {
				...
				Description: `The org and space GUID that is valid for login.`,
			},
		},
...
type cfRole struct {
	...
	BoundGUIDs []string                      `json:"bound_guids"`
	BoundCIDRs []*sockaddr.SockAddrMarshaler `json:"bound_cidrs"`
}

// role takes a storage backend and the name and returns the role's storage
// entry
func (b *cfAuthBackend) role(ctx context.Context, s logical.Storage, name string) (*cfRole, error) {
...
func (b *cfAuthBackend) pathRoleExistenceCheck(ctx context.Context, req *logical.Request, data *framework.FieldData) (bool, error) {
...
func (b *cfAuthBackend) pathRoleList(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
...
func (b *cfAuthBackend) pathRoleRead(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
...
func (b *cfAuthBackend) pathRoleDelete(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
	roleName := data.Get("name").(string)
	if roleName == "" {
...
// of an existing role
func (b *cfAuthBackend) pathRoleCreateUpdate(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
	roleName := data.Get("name").(string)
	if roleName == "" {
	...
	if boundGUIDs, ok := data.GetOk("bound_guids"); ok {
		parsedGUIDs, err := parseutil.ParseCommaStringSlice(boundGUIDs)
		if err != nil {
			return logical.ErrorResponse(err.Error()), nil
		}
		role.BoundGUIDs = parsedGUIDs
	}
...
var roleHelp = map[string][2]string{
	...
	"role": {
		...
		CF token information with token policies and settings.
